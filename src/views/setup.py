...
def add_instance(servers: list[dict[str,str]]):
    ...
    while (not url.startswith("http")) or requests.get(url + "/help_links", timeout=5).status_code != 200:
        url = inquirer.text(message="That dosen't seem like valid canvas URL. Remember to include https and remove the trailing slash:").execute()
    utils.set_current_instance({"name": "setup", "url": url, "token": ""})
    token, token_test_resp = utils.request_token(url)
    name = inquirer.text(message=f"Hello, {token_test_resp['json']['first_name']}! Enter a name for this instance:").execute()
